...
describe('civetkern add test', function() {
  before(function() {
    assert(instance.init(cfg, 0, false) === true)
  })
  let fileids
  const genCount = 20000
  it('generate files id',  function() {
    ...
    for (let idx = 0;idx<genCount; ++idx) {
        ...
        let filename = idx + '.' +filetype
        let keyword = ['building']
        if (idx < 1000 || idx >100000) {
            keyword.push('plane')
        }
        willBeAdd.push({
            'id': fileids[idx],
            'meta': [
              {"name":"path","type":"str","value":"C:\\Users\\webberg\\Pictures\\" + filename},
              {"name":"filename","type":"str","value":filename},
