...
function removeGlobalStyle(style) {
  return style.replace(/(html|body),/g, '')
}
 ...
 * @returns {{body: void | string | *, scripts: *[], styles: *[], entry: *}}
 */
export default function processTpl(tpl, baseURI) {
  ...
  let body = template.match(BODY_REGEX)
  if (body) {
    body = getBodyText(body[0])
  }
  return {
    body,
    scripts,
